 ...
 */
 /*
 * Micro-Manager Arduino Device Adapter Firmware (Modificado para DS18B20 en A0)
 * (Resto de las especificaciones de la interfaz sin cambios)
 */
...
// El sensor DS18B20 se conectará al pin A0, que en el comando 41 se identifica como pin '0'.
#define ONE_WIRE_BUS 14 // El pin A0 del Arduino UNO es el pin digital 14.
// Los pines A0-A5 se mapean a 14-19 en el Arduino, pero usaremos el valor 0 en el comando 41.

OneWire oneWire(ONE_WIRE_BUS);
DallasTemperature sensors(&oneWire);

// Variable para almacenar la temperatura leída (global si se necesita en loop)
// Usaremos un entero de 16 bits para enviar el valor (ej. Temperatura * 100)
int16_t temperatureValue = 0;

void readDS18B20() {
  sensors.requestTemperatures();
  // Obtener la temperatura en Celsius como float
  float tempC = sensors.getTempCByIndex(0); 

  // Convertir a un entero de 16 bits para transmisión serial: 
  // Multiplicamos por 100 para mantener dos decimales de precisión.
  // Ejemplo: 25.45°C -> 2545
  temperatureValue = (int16_t)(tempC * 100.0);
}
...
// **************** FIN DE MODIFICACIONES ****************
 ...
 void setup() {
   ...
   // **************** INICIO DE MODIFICACIONES en setup ****************
   sensors.begin();
   // Configurar la resolución si es necesario (ej. 12 bits)
   // sensors.setResolution(12); 
   // **************** FIN DE MODIFICACIONES en setup ****************
   ...
   if (Serial.available() > 0) {
     ...
     switch (inByte) {
       ...
       case 41:
         if (waitForSerial(timeOut_)) {
           int pin = Serial.read();  
           if (pin == 0) { // **Only PIN A0 (pin == 0) behavior has been modified,**
            // **************** Start of modifications in case 41 ****************
            // 1. READ THE TEMPERATURE
            readDS18B20();
            // This value is an int16_t (2 bytes)
            int16_t val = temperatureValue; 
            // 2. Return the serial answer in the expected format by MM
            Serial.write( byte(41));
            Serial.write( pin); // Pin '0'
            // Sending the int16_t (temperature * 100)
            Serial.write( highByte(val));
            Serial.write( lowByte(val));

            // **************** End modifications in case 41 ****************

            } else if (pin >= 1 && pin <=5) {
              // Standard analog reading is kept for the rest of the pins
